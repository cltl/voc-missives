...
public class NafXmiReader implements NafCreator {
    ...
    private void reduceMultipleMatches() {
        ...
        // remove spans that have been assigned to an entity
        alignedEntities.removeAssignedMatches();
        //one last call to reduce multiple matches separated by anchors
        alignedEntities.duplicateMultiLabelEntitiesAndNormalize();

        // going to extreme measures
        alignedEntities.removeSpansCustom(xmi.getRawText());
    ...
    private String coveredText(Entity entity) {
        References r = (References) entity.getReferencesAndExternalReferences().get(0);
        List<Target> targets = r.getSpen().get(0).getTargets();
        Wf wf0 = (Wf) targets.get(0).getId();
        Wf wf1 = (Wf) targets.get(targets.size() - 1).getId();
